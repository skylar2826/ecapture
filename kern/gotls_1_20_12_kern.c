...
static __always_inline struct go_tls_event *get_gotls_event() {
    ...
    event->fd = 0;  // Initialize fd to 0
    bpf_get_current_comm(event->comm, sizeof(event->comm));

    return event;
}

// Extract fd from Go's crypto/tls.Conn structure
// For Go 1.20: tls.Conn.conn (net.Conn interface) -> *net.TCPConn -> conn.fd.pfd.Sysfd
//
// Structure layout (verified with Go 1.20.14 source code and pahole on Caddy 2.7):
// 
// crypto/tls.Conn {
//     conn net.Conn;  // offset 0, size 16 (interface: type ptr + data ptr)
//     ...
// }
// 
// net.TCPConn {
//     conn net.conn;  // offset 0, size 8 (embedded field)
// }
// 
// net.conn {
//     fd *net.netFD;  // offset 0, size 8 (pointer)
// }
// 
// net.netFD {
//     pfd internal/poll.FD;  // offset 0, size 56 (VALUE type, not pointer!)
//     family int;            // offset 56
//     ...
// }
// 
// internal/poll.FD {
//     fdmu fdMutex;  // offset 0, size 16
//     Sysfd int;     // offset 16, size 8  ← This is what we want!
//     pd pollDesc;   // offset 24
//     ...
// }
// 
// internal/poll.fdMutex {
//     state uint64;  // offset 0, size 8
//     rsema uint32;  // offset 8, size 4
//     wsema uint32;  // offset 12, size 4
// }
static __always_inline int extract_fd_from_tls_conn(void *tls_conn_ptr) {
    if (!tls_conn_ptr) {
        debug_bpf_printk("extract_fd: tls_conn_ptr is NULL\n");
        return 0;
    }
    
    debug_bpf_printk("extract_fd: tls_conn_ptr=%p\n", tls_conn_ptr);
    ...
    if (ret < 0 || !net_conn_ptr) {
        debug_bpf_printk("extract_fd: failed to read net.Conn interface data ptr, ret:%d\n", ret);
        return 0;
    }
    debug_bpf_printk("extract_fd: net_conn_ptr=%p\n", net_conn_ptr);
    ...
    if (ret < 0 || !netfd_ptr) {
        debug_bpf_printk("extract_fd: failed to read netfd ptr, ret:%d, net_conn_ptr=%p\n", ret, net_conn_ptr);
        return 0;
    }
    debug_bpf_printk("extract_fd: netfd_ptr=%p\n", netfd_ptr);
    ...
    if (ret < 0) {
        debug_bpf_printk("extract_fd: failed to read Sysfd, ret:%d, netfd_ptr=%p\n", ret, netfd_ptr);
        return 0;
    }
    
    debug_bpf_printk("extract_fd: extracted fd=%lld (0x%llx)\n", fd, fd);
    
    // Sanity check: fd should be a small positive integer (typically < 65536)
    if (fd < 0 || fd > 65536) {
        debug_bpf_printk("extract_fd: invalid fd value %lld, returning 0\n", fd);
        return 0;
    }
...
static __always_inline int gotls_write(struct pt_regs *ctx, bool is_register_abi) {
    ...
    // First argument is the receiver (*tls.Conn)
    ...
    event->fd = extract_fd_from_tls_conn(tls_conn_ptr);
